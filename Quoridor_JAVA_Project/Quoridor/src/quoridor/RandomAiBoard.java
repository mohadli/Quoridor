/*
 * Created by JFormDesigner on Wed Aug 05 15:25:16 IRDT 2020
 */
 ...
 * @author Aref
 */
public class RandomAiBoard extends JFrame{
...
public RandomAiBoard(Game game){
    this.game = game;
    initComponents();
...
private void aiTurn(){
    ...
    while(true){
        String input = RandomAI.str(Game.currentPlayer);
        if(input.contains(" ")){
            //its a wall
            ...
            if(inputGUIWalls(findButton(X, Y))){
                disableBadWalls();
                break;
            }
        } else{
            ...
            int y1 = Integer.parseInt(inputArr[0] + "");
            int x1 = inputArr[1] - (int) 'a';

            Y = 40 * (y1 - 1) + 165;
            X = 40 * x1 + 625;
            if(inputGUIBoxes(findButton(X, Y))){
                checkThings(findButton(X, Y));
                break;
            }
        }
    }
...
//checks for movements and actually move the pawn
private void checkThings(JButton theBox){
...
private boolean inputGUIWalls(JButton theWall){
